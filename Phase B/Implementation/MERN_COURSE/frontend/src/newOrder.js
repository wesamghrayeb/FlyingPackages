...
export default function NewOrder() {
    ...
    const supplierLocation = { latitude: thisSupplier.location.latitude, longitude: thisSupplier.location.longitude };
    ...
    let thisCourier = couriers.find(c=> c.user.userName === courier)
      ...
      const newOrder = {
        ...
        destination: "6435360605935dafaad4e50a",
    ...
    const handleInputChange = (e) => {
      ...
      useEffect(() => { // here we chain multiple requests 
        ...
        axios.get('http://localhost:3000/api/v1/couriers')//get the couriers
        .then(response => {
...
courierDistances.sort((a, b) => a.distance - b.distance);
 ...
 return (
  <div className="form">
    <h1>New Order</h1>
    <p>First fill the fields, then choose which courier you want to take the delivery</p>
    <div className="form-body">
      <div className="destination" style={{ marginBottom: '5px' }}>
        <label className="form__label" htmlFor="id">Destination</label>
        <input className="form__input" type="text" value={destination} onChange={(e) => handleInputChange(e)} id="destination" placeholder="destination" />
      </div>
      <div className="submitDate" style={{ marginBottom: '5px' }}>
        <label className="form__label" htmlFor="submitDate">Date</label>
        <input className="form__input" type="text" value={submitDate} onChange={(e) => handleInputChange(e)} id="submitDate" placeholder="submitDate" />
      </div>
      <div className="submitHour" style={{ marginBottom: '5px' }}>
        <label className="form__label" htmlFor="submitHour">Hour</label>
        <input type="text" name="" id="submitHour" value={submitHour} className="form__input" onChange={(e) => handleInputChange(e)} placeholder="submitHour" />
      </div>
      <div className="customerPhoneNumber" style={{ marginBottom: '5px' }}>
        <label className="form__label" htmlFor="phone">Phone Number</label>
        <input type="text" name="" id="customerPhoneNumber" value={customerPhoneNumber} className="form__input" onChange={(e) => handleInputChange(e)} placeholder="customerPhoneNumber" />
      </div>

      <div className="couriers-dropdown" style={{ marginBottom: '5px' }}>
        <label className="form__label" htmlFor="courier-dropdown">Courier</label>
        <select id="courier-dropdown" className="form__input" value={courier} onChange={(e) => setCourier(e.target.value)}>
          {courierDistances.map(courierDistance => (
            <option key={courierDistance.courier.user.userName} value={courierDistance.courier.user.userName}>{courierDistance.courier.user.userName}</option>
          ))}
        </select>
      </div>
    ...
    <div className="buttons-section">
      <Button component={Link} to="/supplierForm" color="primary" variant="contained" style={{ marginRight: '16px' }}>
        Back to Main Page
      </Button>
